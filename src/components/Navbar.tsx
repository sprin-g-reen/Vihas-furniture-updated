...
import { Search, ShoppingCart, Menu, X, User, Phone, Heart } from 'lucide-react';
        ...
        <div className="container">
          <ul className="flex justify-between py-3 text-sm font-medium overflow-x-auto">
            {categories.map((category, index) => (
              <li key={index}>
                <a href="#" className="hover:text-vihas-primary whitespace-nowrap">{category}</a>
              </li>
            ))}
          </ul>
        </div>
            ...
            <ul className="space-y-4">
              {categories.map((category, index) => (
                <li key={index} className="border-b border-gray-100 pb-2">
                  <a href="#" className="block text-vihas-secondary font-medium">{category}</a>
                </li>
              ))}
            </ul>
